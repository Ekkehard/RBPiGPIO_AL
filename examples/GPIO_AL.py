...
# I/O (GPIO) functionality for all models of the regular Raspberry Pi as well as
# the Raspberry Pi Pico.  Using this module, code should run and use the common
# functionality of the GPIO shared by all Raspberry Pi architectures without
# modifications on all Raspberry Pi architectures.
#
# @par Comments
...
class GPIOError( Exception ):
    """!
    @brief Exception class to be thrown by the GPIO_AL module.
    ...
    the severity with which it was instantiated or None.
    """
    ...
    def severity( self ):
        """!
        @brief Return severity of the error that caused the exception or None
        if none was given.
        """
...
class IOpin( object ):
    ...
    def pin( self ):
        """!
        @brief Works as read-only property to get the Pin associated with this
               class.
...
class I2Cbus( object ):
    """!
    @brief Class to handle I2C bus communication.

    The GPIO Pins on the Raspberry Pi are GPIO 2 for I2C data and GPIO 3 for I2C
    clock for hardware I2C and freely selectable for software I2C (bit banging);
    on the Raspberry Pi Pico, the GPIO Pins for I2C communication are also
    freely selectable under software I2C, they are restricted to GPIO 1 for data
    and GPIO 2 for clock, GPIO 4 for data and GPIO 5 for clock, GPIO 6 for data
    and GPIO 7 for clock, GPIO 9 for data and GPIO 10 for clock, GPIO 11 for
    data and GOPI 12 for clock, GPIO 14 for data and GOPI 15 for clock, GPIO 16
    for data and GPIO 17 for clock, GPIO 19 for data and GPIO 20 for clock,
    GPIO 21 for data and GPIO 22 for clock, GPIO 24 for data and GPIO 25 for
    clock, GPIO 26 for data and GPIO 27 for clock, or GPIO 31 for data and
    GPIO 32 for clock.
    
    Since many targets can be connected on  an I2C bus, one I2Cbus objects must
    be able to handle them all.  Therefore, I2Cbus objects are created one per
    I2C bus, which is uniquely defined by the sda and scl Pins - NOT one such o
    bject per target on that bus.  Every I2C I/O operation therefore needs to be
    given the I2C address of the target this communication is meant for.
    
    On the Raspberries running under Linux-like operating systems, it is 
    mandatory that the user be part of the group i2c to be able to use the I2C 
    bus.  This can be accomplished by issuing the command (in a terminal window)
    @code
        sudo usermod -a -G i2c $USER
    @endcode
    and then logging out and back in again.  Otherwise, access to the I2C device
    requires elevated privileges (sudo), and the practice of using those when 
    not strictly necessary is strongly discouraged.
        
    It is worth noting that the defaults for the operating mode are different
    between the Raspberry Pi and the Raspberry Pi Pico.  This is because the
    Broadcomm BCM2835 chip, which the Raspberry Pi 3 uses for hardware I2C, is
    broken and does not (reliably) support clock stretching when requested by a
    target.  This was found through measurements and also confirmed online at
    https://www.advamation.com/knowhow/raspberrypi/rpi-i2c-bug.html.  The
    software mode supports clock stretching properly on the Raspberry Pi.
    Naturally, the software-generated I2C clock on a non-real-time OS is not
    very consistent, but targets will tolerate a non-consistent clock a lot
    better than a broken clock-stretch mechanism when they need it.  Therefore,
    the default operating mode on a Raspberry Pi is software, but since the
    Raspberry Pi Pico works just fine with hardware I2C, the default operating
    mode there is hardware to free the CPU from the task of generating I2C
    signals.  This software still allows the caller to select hardware mode also
    on the Raspberry Pi, but the user is strongly advised to make sure that no
    target on the I2C bus requires clock stretching in such cases.
    """

    ## Operate I2C bus in harddware mode
    HARDWARE_MODE = 0
    ## Operate I2C bus in software (bit banging) mode
    SOFTWARE_MODE = 1
    ...
    ## Default frequency for I2C bus communicationsis 100 kHz
    ...
    def __init__( self,
        ...
        @param sdaPin GPIO Pin number for I2C data (default 1 on Raspberry Pi
               and 8 on Raspberry Pi Pico)
        @param sclPin GPIO Pin number for I2C clock (default 2 on Raspberry Pi
               and 9 on Raspberry Pi Pico)
        @param frequency I2C frequency in Hz (default 100 kHz)
    ...
    def __del__( self ):
        ...
        Closes the software I2C bus on the Raspberry Pi.
        """
        self.close()
        return
    ...
    def __setupRP( self ):
        """!
        @brief Private method to set up hardware of the Raspberry Pi as well as
        lambda functions to read from and write to an I2C bus.
        """
    ...
    def __setupRPPico( self ):
        """!
        @brief Private method to set up hardware of the Raspberry Pi Pico as
               well as lambda functions to read from and write to an I2C bus.
        """
        if self.__mode == self.SOFTWARE_MODE:
    ...
    def frequency( self ):
        """!
        @brief Works as read-only property to get the frequency the I2C bus is
               operating at in Hz.
        """
    ...
    def readByte( self, i2cAddress ):
        """!
        @brief Read a single general byte from an I2C device.
        @param i2cAddress address of I2C device to be read from
    ...
    def writeByte( self, i2cAddress, value ):
        ...
        @param i2cAddress address of I2C device to be written to
    ...
    def writeBlockReg( self, i2cAddress, register, block ):
        """!
        @brief Write a block of bytes to an I2C device starting at register.
        @param i2cAddress address of I2C device to be written to
        @param register device register to start reading
