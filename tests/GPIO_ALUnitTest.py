    ...
    def main():
        ...
        try:
            while True:
                ...
                print( 'regular input mode  {0}'.format( PinIO.Mode.INPUT ) )
                print( 'input with pullup   {0}'
                       .format( PinIO.Mode.INPUT_PULLUP ) )
                print( 'input with pulldown {0}'
                       .format( PinIO.INPUT_PULLDOWN ) )
                print( 'output mode         {0}'.format( PinIO.Mode.OUTPUT ) )
                print( 'open drain          {0}
                       .format( PinIO.Mode.OPEN_DRAIN ) )
                mode = int( input( '> ') )
                answer = input( 'Trigger interrupt [y/N]: ' )
                if answer == 'y' or answer == 'Y':
                    isr = callback
                    print( 'Trigger on' )
                    print( 'falling edge {0}'.format( PinIO.FALLING ) )
                    print( 'rising edge  {0}'.format( PinIO.RISING ) )
                    edge = int( input( '> ' ) )
                else:
                    isr = None
                    edge = None
                PinIO = PinIO( pin, mode, callback=isr, edge=edge )
                print( '\nPin is: {0}\n'.format( PinIO ) )
                try:
                    while True:
                        if mode < PinIO.OUTPUT:
